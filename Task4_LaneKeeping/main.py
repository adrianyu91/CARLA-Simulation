...
def main():
    ...
    # Controller with higher Kp for debug
    controller = ProportionalController(Kp=0.03)
    ...
    try:
        while running:
            if hasattr(camera, "last_frame") and camera.last_frame is not None:
                ...
                annotated, binary_lane = detect_lanes(frame, return_binary=True)

                lane_center = estimate_lane_center(binary_lane)
